...
void DtInfo::GetDtRules(vector<Rule*> &_rules) { 
    ...
    sort(_rules.begin(), _rules.end(), [](Rule* a, Rule* b)->bool {return a->pri < b->pri; });
    for (int i = 0; i < rules_num; ++i) {
        DtRule *rule = (DtRule*)malloc(sizeof(DtRule));
        rule->src_dst_ip = (uint64_t)(_rules[i]->ip.i_32.sip & maskBit[_rules[i]->sip_length]) << 32 | (_rules[i]->ip.i_32.dip & maskBit[_rules[i]->dip_length]);
